...
class Webshop:
    ...
    # To-DO: Task 1
    _subEvery: list[Customer] = []

    def addSubEvery(self, new_customer: Customer):
        if new_customer not in self._subEvery:
            self._subEvery.append(new_customer)

    def remSubEvery(self, p_customer: Customer):
        if p_customer in self._subEvery:
            self._subEvery.remove(p_customer)

    def notifySubEvery(self, p_objectOfInterest):
        for sub in self._subEvery:
            sub.update(p_objectOfInterest)
...
# /
    ...
    def addProduct(self, new_product: Product):
        ...
        # TO-DO: Task 1
        if new_product.availibility == True:
            self.notifySubEvery(new_product)
